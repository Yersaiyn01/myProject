...
export const RegisterModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const getRegistrationError = (caught: unknown) => {
    if (axios.isAxiosError(caught)) {
      if (!caught.response) return 'Cannot reach backend server';
      ...
      if (caught.response.status === 400) return 'Email is invalid or already registered';
    }
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        {!successful ? (
          <>
            <h2 className="modal-title">Welcome! How would you like to connect?</h2>
            ...
            <div className="divider">
              <button
                ...
                onClick={() => { window.location.href =  'https://myproject-2y95.onrender.com/auth/login/apple-id/'; }}
              >
              ...
              <button
                ...
                onClick={() => { window.location.href = 'https://myproject-2y95.onrender.com/oauth2/authorization/google'; }}
              >
            ...
            <div className="sales-section">
              <p className="sales-text">Want to talk to our sales team instead?</p>
            ...
            {error && <div className="alert-warning">Signup failed! {error}</div>}
          </>
        ) : (
          <div className="alert-success">Your registration is successful!</div>
        )}
      </div>
    </div>
  );
